      ...
      <div class="max-w-7xl mx-auto px-6 py-4">
        <div class="flex items-center justify-between">
          <div class="flex items-center space-x-4">
            ...
            <div>
              <h1 class="text-2xl font-bold text-gray-800">Estoque Patrimônio</h1>
              <div class="flex items-center text-sm text-gray-600 mt-1">
                <span class="text-gray-400">Menu</span>
                <ChevronRight class="w-4 h-4 mx-1" />
                <span class="text-gray-400">Menu Patrimônio</span>
                <ChevronRight class="w-4 h-4 mx-1" />
                <span>Estoque Patrimônio</span>
                <ChevronRight class="w-4 h-4 mx-1" />
          ...
          <div class="bg-green-50 px-3 py-1 rounded-full">
            <span class="text-green-700 font-medium">{{ totalEstoqueContagem }} produtos no total</span>
          </div>

          <div v-if="categoriaSelecionada" class="bg-blue-50 px-3 py-1 rounded-full">
            <span class="text-blue-700 font-medium">
              {{ totalCategoriaContagem }} produtos em "{{ categoriaSelecionada.nome }}"
            </span>
          </div>
        </div>
        ...
        <div class="p-6">
          ...
          <div class="mt-6">
            ...
            <button
              ...
              :key="categoria.categoria_patrimonio_id"
              @click="selecionarCategoria(categoria)"
              :class="[
                'w-full flex items-center justify-between px-3 py-2 rounded-lg text-left transition-colors mb-1',
                categoriaSelecionada?.categoria_patrimonio_id === categoria.categoria_patrimonio_id ? 'bg-blue-50 text-blue-700 border border-blue-200' : 'text-gray-700 hover:bg-gray-50'
              ]"
            >
              <div class="flex items-center space-x-3">
      ...
      <div class="flex-1 p-6">
        ...
        <div class="flex items-center justify-between mb-6">
          ...
          <div class="flex items-center space-x-4">
            ...
            <button
            @click="mostrarModalMovimentacoes = true"
            class="bg-blue-500 hover:bg-blue-600 text-white font-medium py-2 px-4 rounded-lg transition-colors flex items-center space-x-2"
          >
        ...
        <ModalAdicionarProduto v-if="mostrarModalProduto" :categorias="categorias" :localizacoes="localizacoes" @fechar="mostrarModalProduto = false"
          @salvou="carregarProdutos(categoriaSelecionada?.categoria_patrimonio_id, currentPage)" />
          <ModalAdicionarCategoria v-if="mostrarModal" @fechar="mostrarModal = false" @salvou="categoriaSalva" />
          <Loading v-if="loadingControl"/>
          <ModalEditarProduto v-if="mostrarModalEdicao" :categorias="categorias" :produto="produtoSelecionado" :localizacoes="localizacoes" @fechar="mostrarModalEdicao = false" @atualizado="handleProdutoAtualizado" />
          <ModalExcluirDecoracao v-if="mostrarModalExclusao" :produto="produtoParaExcluir" @cancelar="cancelarExclusao" @excluido="handleProdutoExcluido" />
          <ModalVisualizarPatrimonio 
            v-if="mostrarModalVisualizacao" 
            :produto="produtoParaVisualizar"
            :categorias="categorias"
            :localizacoes="localizacoes"
            @fechar="mostrarModalVisualizacao = false"
            @editar="abrirModalEdicaoDeVisualizacao"
          />
          <ModalMovimentacoes
            v-if="mostrarModalMovimentacoes"
            :isOpen="mostrarModalMovimentacoes"
            @close="mostrarModalMovimentacoes = false"
          />
...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mb-8 pb-10">
  ...
  <div
    ...
    :key="peca.produtos_patrimonio_id"
    @click="abrirModalVisualizacao(peca)"
    ...
    <div class="relative h-48 bg-gray-100">
      ...
      <div class="absolute top-2 left-2" @click.stop>
        <div class="relative">
          ...
          <button
            @click.stop="toggleDropdown(peca.produtos_patrimonio_id)"
          ...
          <!-- Dropdown Menu -->
          <div 
            v-if="dropdownAberto === peca.produtos_patrimonio_id"
    ...
    <div class="p-4">
      ...
      <div class="space-y-2 text-sm mb-3">  
        <div class="flex justify-between">
          ...
          <span class="text-amber-600 font-medium">{{ peca.numero_identificacao}}</span>
        </div>
      </div>
    </div>
  </div>
...
import ModalAdicionarCategoria from '@/components/ModalAdicionarCategoriaPatrimonio.vue'
import ModalAdicionarProduto from '@/components/ModalAdicionarPatrimonio.vue'
import Paginacao from '@/components/Paginacao.vue'
import { Folder, Plus, Search, Image, ArrowLeft, ChevronRight, Edit3, EyeOff, Eye, Trash2, MoreVertical, History } from 'lucide-vue-next'
import Loading from '../components/Loading.vue'
import ModalEditarProduto from '../components/ModalEditarPatrimonio.vue'
import ModalExcluirDecoracao from '../components/ModalExcluirPatrimonio.vue'
import jsPDF from 'jspdf';
//import { reactive } from 'vue'
import autoTable from 'jspdf-autotable';
//import 'jspdf-autotable';
import imgPdf from '@/assets/smip.png';
import ModalVisualizarPatrimonio from '../components/ModalVisualizarPatrimonio.vue'
import ModalMovimentacoes from '@/components/ModalMovimentacoes.vue';
...
export default {
  ...
  data() {
    return {
      ...
      totalCategoriaContagem: 0,
      menuExpansivel: null,
      debouncedBuscarProdutos: () => { },
      mostrarModalVisualizacao: false,
      mostrarModalMovimentacoes: false
    };
  },
  computed: {
    categoriasFiltradas() {
  ...
  mounted() {
    ...
    this.debouncedBuscarProdutos = debounce(() => {
      this.carregarProdutos(this.categoriaSelecionada?.categoria_patrimonio_id, 1);
    }, 500);
  },
  methods: {

    abrirModalVisualizacao(produto) {
      this.produtoParaVisualizar = {...produto};
    ...
    async generatePDF() {
      ...
      try {
        ...
        const response = await axios.get('/patrimonio-ativos');
        const patrimoniosAtivos = response.data;
        ...
        const tituloRelatorio = "RELATÓRIO DE PATRIMÔNIOS ATIVOS";
        ...
        didDrawPage: (data) => {
          const numeroDaPagina = doc.internal.getNumberOfPages();
          const total = patrimoniosAtivos.length;
    ...
    async totalEstoque() {
      try {
        ...
        if (this.categoriaSelecionada) {
          params.categoria_id = this.categoriaSelecionada.categoria_patrimonio_id;
        }

        const response = await axios.get('/total-estoque', { params });
        this.totalEstoqueContagem = response.data.total_estoque;
        this.totalCategoriaContagem = response.data.total_categoria;
      } catch (error) {
    ...
    async carregarCategorias() {
      try {
        const response = await axios.get('/categorias-patrimonio');
    ...
    async carregarLocalizacoes() {
      try {
        const response = await axios.get('/localizacoes'); // 👈 mesma rota criada no backend
    ...
    async carregarProdutos(categoriaId = null, page = 1) {
      ...
      try {
        ...
        const response = await axios.get('/produtos-patrimonio', { params });
    ...
    selecionarCategoria(categoria) {
      ...
      this.totalEstoque(); //testando
      this.currentPage = 1;
      if (categoria) {
        this.carregarProdutos(categoria.categoria_patrimonio_id, 1);
      } else {
    ...
    mudarPagina(page) {
      if (page >= 1 && page <= this.lastPage) {
        ...
        this.carregarProdutos(this.categoriaSelecionada?.categoria_patrimonio_id, this.currentPage);
      }
    },
  ...
  async toggleStatusProduto(peca, novoStatus) {
    ...
    console.log("ID: ", peca.produtos_patrimonio_id)
    console.log("status atual: ", peca.ativo)
    console.log("Novo status: ", novoStatus)
    try{
      const response = await axios.put(`/mudar-ativo-patrimonio/${peca.produtos_patrimonio_id}`, {
        ativo: novoStatus
      })
