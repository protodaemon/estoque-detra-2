      ...
      <div class="bg-gradient-to-r from-blue-300 to-blue-600 p-6 text-white relative shrink-0 rounded-2xl">
        ...
        <div class="relative z-10">
          <div class="flex items-center gap-3 mb-2">
            ...
            <h2 class="text-2xl font-bold">Registro de Consumível</h2>
          </div>
          <p class="text-white/80 text-sm">Adicione um novo consumível público</p>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    async salvarProduto() {
      ...
      this.salvando = true;
      console.log("teste 1:", this.form.categoria_consumivel);
      ...
      formData.append('categoria_consumivel', parseInt(this.form.categoria_consumivel));
      ...
      try {
        // console.log("teste 2:", formData);
        const response = await axios.post('produtos-consumivel', formData);
        const produtoId = response.data.produtos_consumivel_id;
        console.log("teste response:", response.data);
        this.$emit('salvou');
        this.$emit('fechar');
      } catch (error) {
        console.error("Erro ao salvar produto ou entrada:", error);
