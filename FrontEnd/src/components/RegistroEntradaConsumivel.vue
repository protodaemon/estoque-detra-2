...
<!--           <button @click="goBack"
          ...
          <div>
            <h1 class="text-2xl font-bold">Registrar Entrada</h1>
      ...
      <div v-if="selectedProduto" class="mt-6 border-t border-gray-200 bg-gray-50 p-6 rounded-xl">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <p class="text-xs text-gray-500 mt-1">Novo estoque: {{ selectedProduto.qtd_disponivel + quantidade }} unidades</p>
          </div>

          <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">Data da entrada</label>
            <input v-model="dataEntrada" type="date" class="w-full py-2 px-4 border-2 border-gray-200 rounded-lg focus:border-blue-400 focus:ring-4 focus:ring-blue-100 transition-all" />
          </div>
...
export default {
  name: 'RegistroEntradaConsumivelPage',
  ...
  methods: {
    ...
    async registrarEntrada() {
      ...
      this.loading = true
      try {
        const entradaData = this.activeTab === 'decoracao'
          ? {
