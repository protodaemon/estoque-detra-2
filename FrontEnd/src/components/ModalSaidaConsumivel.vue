...
<!--           <button @click="goBack" class="p-2 bg-white/10 rounded-lg hover:bg-white/20 transition-colors">
          ...
          <div>
            <h1 class="text-2xl font-bold">Histórico de Saidas</h1>
            <p class="text-white/80 text-sm">Visualize todas as saidas registradas no estoque</p>
          </div>
        </div>
      </div>
    </header>
      ...
      <div v-else class="bg-white rounded-xl border border-gray-200 overflow-hidden">
        ...
        <div class="max-h-96 overflow-y-auto">
          ...
          >
            <div class="grid grid-cols-6 gap-4 items-center">
              <!-- Data -->
              <div class="text-sm">
                <p class="font-medium text-gray-800">{{ formatarData(saida.data_saida) }}</p>
              </div>
              ...
              <div class="flex items-center gap-3">
                <div class="w-10 h-10 bg-gray-100 rounded-lg flex items-center justify-center flex-shrink-0">
                  <img
                    v-if="saida.produto?.foto"
                    :src="saida.produto.foto"
                    :alt="saida.produto.nome"
                    class="w-full h-full object-cover rounded-lg"
                  />
                  <Package v-else class="w-4 h-4 text-gray-400" />
                </div>
                <div class="min-w-0">
                  <p class="font-medium text-gray-800 truncate">{{ saida.produto?.nome }}</p>
                  <p class="text-xs text-gray-500">{{ saida.categoria_nome }}</p>
                </div>
              </div>
              ...
              <div class="text-sm col-span-2">
                <p class="text-gray-600 truncate" :title="saida.observacao">
                  {{ saida.observacao || '-' }}
                </p>
              </div>
            </div>
        ...
        <div v-if="historico.length === 0 && !loadingHistorico" class="text-center py-12">
          ...
          <p class="text-gray-500 mb-2">
            {{ searchTerm ? 'Nenhuma saida encontrada' : 'Nenhuma saida registrada' }}
          </p>
          <p class="text-sm text-gray-400">
            {{ searchTerm ? 'Tente ajustar o termo de busca' : 'As saidas aparecerão aqui quando forem registradas' }}
          </p>
        </div>
      </div>
...
import { History, X, Search, Package, Plus, Minus, Flower } from 'lucide-vue-next'
  ...
  methods: {
    ...
    async carregarHistorico(page = 1) {
      ...
      try {
        ...
        const response = await axios.get(`/saida-${this.activeTab}`, { params })
