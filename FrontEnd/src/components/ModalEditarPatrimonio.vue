      ...
      <div class="bg-gradient-to-r from-purple-400 to-purple-600 p-6 text-white relative">
        ...
        <div class="relative z-10">
          ...
          <p class="text-white/90 text-sm mt-1">
            Número de Identificação: <span class="font-semibold">{{ form.numero_identificacao }}</span>
          </p>
          ...
          <label class="flex items-center gap-2 text-sm font-semibold text-gray-700">
            <svg class="w-4 h-4 text-purple-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                d="M17.657 16.657L13.414 12.414a2 2 0 00-2.828 0L6.343 16.657a8 8 0 1111.314 0zM12 10a2 2 0 110-4 2 2 0 010 4z"/>
            </svg>
            Localização
          </label>
...
export default {
  ...
  props: ['categorias', 'produto', 'localizacoes'],
  data() {
    return {
      form: {
  ...
  methods: {
    preencherFormulario() {
      if (this.produto) {
        this.form.categoria_patrimonio = this.produto.categoria_patrimonio || '';
        this.form.descricao = this.produto.descricao || '';
        this.form.localizacao_id = this.produto.localizacao_id || '';
    ...
    async atualizarProduto() {
      ...
      if (!this.form.localizacao_id || !this.form.categoria_patrimonio || !this.form.descricao) {
        alert("Preencha todos os campos obrigatórios!");
        return;
      }
      /*
      // Validação do número de identificação
      if (this.form.numero_identificacao.length !== 5) {
        alert("O número de identificação deve ter exatamente 5 dígitos!");
        return;
      }*/
      ...
      try {
        const response = await axios.post(`editar-patrimonio/${this.produto.produtos_patrimonio_id}`, formData, {
          headers: {
